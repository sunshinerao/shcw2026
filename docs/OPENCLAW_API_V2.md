...
1. 路径前缀替换：将 `/api/v1/` 全量替换为 `/api/v2/`。
2. 认证与权限不变：仍使用 API Key（`Authorization: Bearer` 或 `x-api-key`），scope 与 V1 一致。
3. 请求/响应结构不变：字段、分页、错误码与 V1 保持一致。
4. 建议迁移顺序：先读接口（GET）再写接口（POST/PATCH/PUT/DELETE），最后切换生产流量。
...
所有 `/api/v2/` 接口均需携带 API 密钥，支持两种方式（任选其一）：
...
`code` 为 V2 标准错误码字段。若下游业务未显式提供 `code`，V2 网关层将按 HTTP 状态自动补齐（如 401->`UNAUTHORIZED`、403->`FORBIDDEN`、500->`INTERNAL_ERROR`）。
...
```http
GET /api/v2/events?page=1&pageSize=10&type=forum&published=true
Authorization: Bearer sk_oc_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
```
...
返回该活动的报名观众列表，包含用户姓名、邮箱、手机、职务、所属机构摘要、角色、气候护照 ID，以及当前活动的已批准人数统计。
...
```
GET /api/v2/events/{eventId}/agenda
```
...
```
POST /api/v2/events/{eventId}/agenda
Content-Type: application/json
```
...
```
GET /api/v2/events/{eventId}/agenda/{agendaId}
```
...
```
PUT /api/v2/events/{eventId}/agenda/{agendaId}
Content-Type: application/json
```
...
```
DELETE /api/v2/events/{eventId}/agenda/{agendaId}
```
...
```
GET /api/v2/insights/{id_or_slug}
```
...
```
PATCH /api/v2/insights/{id_or_slug}
Content-Type: application/json
```
...
```
DELETE /api/v2/insights/{id_or_slug}
```
...
# 查询已发布活动（第1页，每页10条）
resp = httpx.get(f"{BASE_URL}/api/v2/events", params={"page": 1, "pageSize": 10}, headers=headers)
events = resp.json()["data"]

# 创建活动
new_event = httpx.post(f"{BASE_URL}/api/v2/events", json={
    "title": "净零科技论坛",
...
httpx.post(f"{BASE_URL}/api/v2/events/{event_id}/agenda", json={
    "title": "开场致辞",
