...
async function getExistingIds<T extends { id: string }>(
  load: () => Promise<T[]>,
  ids: string[],
) {
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [validEventIds, validInstitutionIds, validSpeakerIds, validTrackIds] = await Promise.all([
      Array.isArray(relatedEventIds)
        ...
        : Promise.resolve([]),
      Array.isArray(relatedInstitutionIds)
        ? getExistingIds(
            () => prisma.institution.findMany({
              where: { id: { in: normalizedInstitutionIds } },
              select: { id: true },
            }),
            normalizedInstitutionIds,
          )
        : Promise.resolve([]),
      Array.isArray(relatedSpeakerIds)
        ? getExistingIds(
    ...
    const effectiveTitle = String(updateData.title ?? existingAsset.title ?? "").trim();
    const effectiveSubtitle = String(updateData.subtitle ?? existingAsset.subtitle ?? effectiveTitle ?? "").trim();
    const effectiveSummary = String(updateData.summary ?? existingAsset.summary ?? "");
    const effectiveContent = String(updateData.content ?? existingAsset.content ?? "");
