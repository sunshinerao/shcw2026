...
/**
 * POST /api/news/seed — Admin only: seed 10 sample news articles.
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: { created, skipped, total: newsItems.length },
    });
  } catch (error) {
    console.error("Seed news error:", error);
    return NextResponse.json(
      { success: false, error: "Failed to seed news: " + (error instanceof Error ? error.message : String(error)) },
      { status: 500 }
    );
  }
}
