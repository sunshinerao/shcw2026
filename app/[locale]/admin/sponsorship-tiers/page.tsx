...
export default function AdminSponsorshipTiersPage() {
  const t = useTranslations("adminSponsorshipTiersPage");
  ...
  const loadingLabel = locale === "en" ? "Loading sponsorship tiers..." : "正在加载赞助方案...";
  const genericLoadError = locale === "en" ? "Failed to load sponsorship tiers." : "加载赞助方案失败。";
  ...
  const loadTiers = useCallback(async () => {
    ...
    try {
      ...
      setTiers(data.data as SponsorshipTier[]);
      setStatusMessage("");
    } catch (error) {
  ...
  // 打开删除对话框
  const handleDeleteClick = (tier: SponsorshipTier) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/sponsorship-tiers/${deletingTier.id}`, {
        method: "DELETE",
      });
      const data = await response.json();
      ...
      toast.success(data.message || "Sponsorship tier deleted successfully");
  ...
  const handleSave = async () => {
    ...
    const url = editingTier
      ? `/api/sponsorship-tiers/${editingTier.id}`
    ...
    try {
      ...
      toast.success(data.message || "Sponsorship tier saved successfully");
  ...
  // 切换激活状态
  const toggleTierStatus = async (tier: SponsorshipTier) => {
    try {
      const response = await fetch(`/api/sponsorship-tiers/${tier.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
        ...
        >
          <div>
            <h1 className="text-2xl font-bold text-slate-900 mb-2">
              {locale === "en" ? "Sponsorship Tiers" : "赞助方案管理"}
            </h1>
            <p className="text-slate-600">
              {locale === "en"
                ? "Manage sponsorship tier details and features"
                : "管理赞助方案的详情和特性"}
            </p>
            {statusMessage && (
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Sponsorship tier description"
                    rows={2}
                  />
                </div>
              </div>
